...
function handleRpcError(err) {
  if (err instanceof ConnectError2) {
    switch (err.code) {
      case Code2.InvalidArgument:
        return new InvalidArgumentError(err.message);
      case Code2.Unauthenticated:
        return new AuthenticationError(err.message);
      case Code2.NotFound:
        return new NotFoundError(err.message);
      case Code2.Unavailable:
...
function authenticationHeader(envdVersion, username) {
  if (username == void 0 && compareVersions(envdVersion, ENVD_DEFAULT_USER) < 0) {
...
var file_filesystem_filesystem = /* @__PURE__ */ fileDesc(
...
var Filesystem = /* @__PURE__ */ serviceDesc(file_filesystem_filesystem, 0);
...
var Filesystem2 = class {
  constructor(transport, envdApi, connectionConfig) {
    ...
    this.rpc = createClient3(Filesystem, transport);
  }
  async read(path2, opts) {
    var _a3;
    const format = (_a3 = opts == null ? void 0 : opts.format) != null ? _a3 : "text";
    let user = opts == null ? void 0 : opts.user;
    if (user == void 0 && compareVersions2(this.envdApi.version, ENVD_DEFAULT_USER) < 0) {
  ...
  async makeDir(path2, opts) {
    try {
      ...
      if (err instanceof ConnectError3) {
        if (err.code === Code3.AlreadyExists) {
          return false;
        }
      }
      throw handleRpcError(err);
...
import {
  Code as Code5,
  ConnectError as ConnectError5,
  createClient as createClient5
} from "@connectrpc/connect";
import { compareVersions as compareVersions3 } from "compare-versions";

// src/envd/process/process_pb.ts
import {
  enumDesc as enumDesc2,
  fileDesc as fileDesc2,
  messageDesc as messageDesc2,
  serviceDesc as serviceDesc2
} from "@bufbuild/protobuf/codegenv2";
var file_process_process = /* @__PURE__ */ fileDesc2(
...
var Process = /* @__PURE__ */ serviceDesc2(file_process_process, 0);

// src/sandbox/commands/pty.ts
import {
  Code as Code4,
  ConnectError as ConnectError4,
  createClient as createClient4
} from "@connectrpc/connect";
var Pty = class {
  constructor(transport, connectionConfig, metadata) {
    this.transport = transport;
    this.connectionConfig = connectionConfig;
    this.rpc = createClient4(Process, this.transport);
  ...
  async kill(pid, opts) {
    try {
      ...
      if (err instanceof ConnectError4) {
        if (err.code === Code4.NotFound) {
          return false;
        }
      }
      throw handleRpcError(err);
...
var Commands = class {
  constructor(transport, connectionConfig, metadata) {
    ...
    this.rpc = createClient5(Process, transport);
    this.envdVersion = metadata.version;
  }
  /**
  ...
  async kill(pid, opts) {
    try {
      ...
      if (err instanceof ConnectError5) {
        if (err.code === Code5.NotFound) {
          return false;
        }
      }
      throw handleRpcError(err);
  ...
  async start(cmd, opts) {
    ...
    if ((opts == null ? void 0 : opts.stdin) === false && compareVersions3(this.envdVersion, ENVD_COMMANDS_STDIN) < 0) {
...
var Sandbox = class extends SandboxApi {
  ...
  async getMetrics(opts) {
    ...
    if (this.envdApi.version) {
      if (compareVersions5(this.envdApi.version, "0.1.5") < 0) {
        throw new SandboxError(
          "You need to update the template to use the new SDK. You can do this by running `e2b template build` in the directory with the template."
        );
      }
      if (compareVersions5(this.envdApi.version, "0.2.4") < 0) {
